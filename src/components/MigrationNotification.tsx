import React, { useState, useEffect } from 'react'

interface MigrationNotificationProps {
  onDismiss: () => void
}

const MigrationNotification: React.FC<MigrationNotificationProps> = ({ onDismiss }) => {
  const [isVisible, setIsVisible] = useState(false)

  useEffect(() => {
    // Check if migration was recently completed
    const migrationCompleted = localStorage.getItem('quarterback-migration-completed')
    if (migrationCompleted) {
      setIsVisible(true)
      // Remove the flag after showing the notification
      localStorage.removeItem('quarterback-migration-completed')
    }
  }, [])

  if (!isVisible) return null

  return (
    <div className="fixed top-4 right-4 z-50 max-w-md">
      <div className="bg-green-50 border border-green-200 rounded-lg p-4 shadow-lg">
        <div className="flex items-start">
          <div className="flex-shrink-0">
            <svg className="h-5 w-5 text-green-400" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
            </svg>
          </div>
          <div className="ml-3 flex-1">
            <h3 className="text-sm font-medium text-green-800">
              Data Migration Completed
            </h3>
            <div className="mt-1 text-sm text-green-700">
              <p>
                Your existing data has been migrated to the new quarter-centric architecture. 
                All items, team members, and holidays have been assigned to your current quarter.
              </p>
            </div>
            <div className="mt-3">
              <button
                onClick={() => {
                  setIsVisible(false)
                  onDismiss()
                }}
                className="text-sm bg-green-100 text-green-800 px-3 py-1 rounded hover:bg-green-200 transition-colors"
              >
                Got it
              </button>
            </div>
          </div>
          <div className="ml-4 flex-shrink-0">
            <button
              onClick={() => {
                setIsVisible(false)
                onDismiss()
              }}
              className="text-green-400 hover:text-green-600"
            >
              <svg className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
              </svg>
            </button>
          </div>
        </div>
      </div>
    </div>
  )
}

export default MigrationNotification
